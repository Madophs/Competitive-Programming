...
void decode_huff(node * root,string s,int pos=0,node * originalRoot = nullptr ) {
    if(originalRoot==NULL)originalRoot = root;
    if(root->data!=0){
        cout<<root->data;
        if(pos < s.length()){
          decode_huff(originalRoot, s, pos, originalRoot);
        }
    }else{
        if(pos < s.length()){
            if(s.at(pos)=='1'){
                decode_huff(root->right,s,pos+1,originalRoot);
            }else{
                decode_huff(root->left, s, pos + 1,originalRoot);
            }
        }
    }
}
