...
BigInteger BigInteger::multiplication(BigInteger big1, BigInteger big2){
	BigInteger ONE(0),ZERO(0);
	if(big1 == ZERO || big2 == ZERO) return ZERO;
	if(big1 == ONE) return big2;
	if(big2 == ONE) return big1;
	if(big1.length() + big2.length() <= 16){
		long long val1 = atoll(big1.toString().c_str()), val2 = atoll(big2.toString().c_str());;
